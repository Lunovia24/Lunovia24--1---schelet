...
const compliments = [
    ...
    // Adaugă mai multe complimente, dacă vrei
];
...
// Alegem 1 compliment corect și 3 greșite
const selectedCompliments = [];
selectedCompliments.push(correctCompliments[Math.floor(Math.random() * correctCompliments.length)]); // 1 compliment corect

// Alegem 3 complimente greșite
for (let i = 0; i < 3; i++) {
    selectedCompliments.push(wrongCompliments[Math.floor(Math.random() * wrongCompliments.length)]);
}
